'use strict';
// Seeded RNG, integer hashing and simplex noise (2D / 3D).
...
function hash3(x, y, z, seed) {
  let h = (seed | 0) ^ Math.imul(x | 0, 0x27d4eb2d) ^ Math.imul(y | 0, 0x165667b1) ^ Math.imul(z | 0, 0x9e3779b1);
...
// Tileable value-noise fbm on a size x size torus, 4 channels at different frequencies.
// Used for water normals and clouds on the GPU.
function makeTileableNoise(size, seed) {
  const out = new Uint8Array(size * size * 4);
  const rng = mulberry32(seed);
  const channelFreqs = [4, 8, 16, 32];
  for (let c = 0; c < 4; c++) {
    const acc = new Float32Array(size * size);
    let amp = 1, norm = 0;
    for (let oct = 0; oct < 4; oct++) {
      const f = channelFreqs[c] << oct;
      if (f > size) break;
      const grid = new Float32Array(f * f);
      for (let i = 0; i < grid.length; i++) grid[i] = rng();
      for (let y = 0; y < size; y++) {
        const gy = (y / size) * f, y0 = Math.floor(gy), ty = gy - y0;
        const sy = ty * ty * (3 - 2 * ty);
        const ya = y0 % f, yb = (y0 + 1) % f;
        for (let x = 0; x < size; x++) {
          const gx = (x / size) * f, x0 = Math.floor(gx), tx = gx - x0;
          const sx = tx * tx * (3 - 2 * tx);
          const xa = x0 % f, xb = (x0 + 1) % f;
          const v0 = grid[ya * f + xa] + (grid[ya * f + xb] - grid[ya * f + xa]) * sx;
          const v1 = grid[yb * f + xa] + (grid[yb * f + xb] - grid[yb * f + xa]) * sx;
          acc[y * size + x] += (v0 + (v1 - v0) * sy) * amp;
        }
      }
      norm += amp;
      amp *= 0.5;
    }
    // Normalise to use the full byte range.
    let mn = Infinity, mx = -Infinity;
    for (let i = 0; i < acc.length; i++) { mn = Math.min(mn, acc[i]); mx = Math.max(mx, acc[i]); }
    const range = mx - mn || 1;
    for (let i = 0; i < acc.length; i++) out[i * 4 + c] = Math.round(((acc[i] - mn) / range) * 255);
  }
  return out;
}
